...
class SortableGrid extends Component {
      ...
      >
        { this.state.gridLayout &&
          this.items.map( (item, key) =>
              ...
              onLongPress = { this.activateDrag(item,key) }
  ...
  componentWillReceiveProps = (properties) => this.handleNewProps(properties)
    ...
    handleNewProps = (properties) => {
         ...
         * });
         * if(staticItem){
         *     this._fixItemOrderOnDeletion(staticItem);
         *     this.itemOrder.splice(staticIndex, 1);
         *     let tmp = this.items[staticIndex];
         *     this.items.splice(staticIndex,1);
         *     staticItem.order = this.items.length;
         *     this.items.push(tmp);
         *     //this.deleteBlocks([]);
         * } */
    }
  ...
  onStartDrag = (evt, gestureState) => {
    ...
    if (this.state.activeBlock != null && this._blockPositionsSet()) {
      ...
      if (closest !== this.state.activeBlock && !this.items[closest].props.isStatic) {
        Animated.timing(
  ...
  setGhostPositions = () => {
    ...
    activateDrag = (item,key) =>{
        if(item.props.isStatic){
            return ()=>{};
        }
        return () => {
